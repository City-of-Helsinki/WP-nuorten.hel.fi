...
class Expirator implements Interfaces\Controller {
    ...
    public function hooks() : void {
        // Hook to post saving.
        \add_action( 'save_post', \Closure::fromCallable( [ $this, 'schedule_expiration' ] ), 10, 2 );

        // Add the post expirating method to the corresponding cron event hook.
        \add_action( 'expirate_post', \Closure::fromCallable( [ $this, 'expirate' ] ), 10, 1 );
    }
    ...
    private function schedule_expiration( int $post_id, \WP_Post $post ) : void {
        ...
        \wp_clear_scheduled_hook( 'expirate_post', [ $post_id ] );
        ...
        if ( ! empty( $expiration_time ) ) {
            ...
            \wp_schedule_single_event( $gmt_timestamp, 'expirate_post', [ $post_id ] );
        }
    }
     ...
     * @return void
     */
    private function expirate( $post_id ) {
